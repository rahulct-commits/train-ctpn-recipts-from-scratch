...
class TextProposalConnectorOriented:
    ...
    def get_text_lines(self, text_proposals, scores, im_size):
        ...
        """
        # tp=text proposal
        tp_groups = self.group_text_proposals(text_proposals, scores, im_size) # First of all, it is to build a picture, which small boxes constitute the text line

        text_lines = np.zeros((len(tp_groups), 8), np.float32)

        for index, tp_indices in enumerate(tp_groups):
            text_line_boxes = text_proposals[list(tp_indices)]  # All small boxes for each text line
            X = (text_line_boxes[:, 0] + text_line_boxes[:, 2]) / 2  # Find the x and y coordinates of the center of each small box
            Y = (text_line_boxes[:, 1] + text_line_boxes[:, 3]) / 2 

            z1 = np.polyfit(X, Y, 1)  # Polynomial fitting, fitting a straight line according to the center store found before (least squares)
